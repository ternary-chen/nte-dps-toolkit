...
namespace Settings
{
	...
	namespace Generator
	{
		...
		inline std::filesystem::path SDKGenerationPath;
	}
...
R"(
	...
	namespace NteRuntimeOffsets
	{
		/* Filled at runtime from the embedded find_offsets scanner. */
		inline int32 OffsetGObjects = 0;
		inline int32 OffsetAppendString = 0;
		inline int32 OffsetGNames = 0;
		inline int32 OffsetGWorld = 0;
		inline int32 OffsetProcessEvent = 0;
		inline int32 IndexProcessEvent = 0;
		inline int32 IndexViewportTick = 0;
		constexpr int32 GObjectsChunkSize = 0x10000;
		inline bool (*CancellationRequested)() noexcept = nullptr;

		inline void ThrowIfCancellationRequested()
		{
			if (CancellationRequested != nullptr && CancellationRequested())
				throw std::runtime_error("Dumper-7 SDK generation cancelled");
		}
	}

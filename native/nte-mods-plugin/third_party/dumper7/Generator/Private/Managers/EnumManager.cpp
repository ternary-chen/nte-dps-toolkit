...
namespace EnumInitHelper
{
	...
	void SetEnumSizeForValue(uint8& Size, uint64 EnumValue)
	{
		if (EnumValue > GetMaxOfType<uint32>()) {
			...
			Size = std::max(Size, static_cast<uint8>(0x4));
		}
		else if (EnumValue > GetMaxOfType<uint8>()) {
			Size = std::max(Size, static_cast<uint8>(0x2));
		}
		else {
			Size = std::max(Size, static_cast<uint8>(0x1));
		}
	}
}
		...
		if (!Settings::Internal::bHasUnderlayingTypeInUEnum && Obj.IsA(EClassCastFlags::Struct))
		{
			...
			for (UEProperty Property : ObjAsStruct.GetProperties())
			{
				...
				if (Enum)
				{
					...
					Info.UnderlyingTypeSize = static_cast<uint8>(std::max<int32>(
						Info.UnderlyingTypeSize, PropertySize));
					continue;
				}
		...
		else if (Obj.IsA(EClassCastFlags::Enum))
		{
			...
			for (int i = 0; i < NameValuePairs.size(); i++)
			{
				...
				if (!NameWitPrefix.ends_with(L"_MAX") && Value >= 0)
					EnumMaxValue = std::max(
						EnumMaxValue, static_cast<uint64>(Value));

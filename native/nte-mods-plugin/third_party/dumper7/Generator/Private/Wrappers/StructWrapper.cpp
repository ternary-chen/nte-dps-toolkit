...
{
    if (!IsValid())
        return StructWrapper(UEStruct(nullptr));
...
{
    return IsValid() && (bIsUnrealStruct ? Struct.IsA(EClassCastFlags::Class) : PredefStruct->bIsClass);
}

bool StructWrapper::IsUnion() const
{
    return IsValid() && !bIsUnrealStruct && PredefStruct->bIsUnion;
}

bool StructWrapper::IsFunction() const
{
    return IsValid() && bIsUnrealStruct && Struct.IsA(EClassCastFlags::Function);
}
...
bool StructWrapper::IsInterface() const
{
    ...
    return IsValid() && bIsUnrealStruct && Struct.IsA(EClassCastFlags::Class) && Struct.HasType(InterfaceClass);
}
...
bool StructWrapper::IsExactClassUObject() const
{
    ...
    return IsValid() && bIsUnrealStruct && Struct == UObjectClass;
}

bool StructWrapper::IsAClassWithType(UEClass TypeClass) const
{
    return IsValid() && IsUnrealStruct() && IsClass() && Struct.Cast<UEClass>().IsA(TypeClass);
}
...
bool StructWrapper::IsValid() const
{
    ...
    return bIsUnrealStruct ? static_cast<bool>(Struct) : PredefStruct != nullptr;
}
...
{
    if (!IsValid() || !bIsUnrealStruct || PackageIndex == -1)
        return false;
